...
const updateAnimal = (arr, callback) => {
  const newArr = [];
  arr.forEach((value) => {
    return newArr.push(callback(value));
  });
  return newArr;
};
...
const sortNumbersByLength = (arr) => {
  return arr.sort((a, b) => {
    const aLen = a.toLocaleString().length;
    const bLen = b.toLocaleString().length;
...
const sortPeopleBetter = (arr) => {
  return arr.sort((a, b) => {
    const aFirst = a.firstName.toUpperCase();
    const bFirst = b.firstName.toUpperCase();
